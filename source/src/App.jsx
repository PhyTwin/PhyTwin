...
import { ArrowRight, BriefcaseBusiness, Check, ChevronRight, CircleDot, Code2, Download, ExternalLink, Gauge, Mail, Menu, Pause, Play, RotateCcw, Save, Sparkles, X } from 'lucide-react'
...
const baseLayout = {
  ...
  margin: { l: 54, r: 20, t: 34, b: 48 }, hoverlabel: { bgcolor: '#171916', font: { color: '#fff' } },
}
...
function Shell({ children, interviewer, setInterviewer }) {
  ...
  return <div className={interviewer ? 'interviewer-mode' : ''}>
    <header className="topbar">
      ...
      <div className="header-actions">
        <button className={`interviewer-toggle ${interviewer ? 'active' : ''}`} onClick={() => setInterviewer(!interviewer)}><BriefcaseBusiness size={15} /> 面试官模式</button>
...
function Home({ interviewer }) {
  useDocumentTitle('自研 CAE 在线实时仿真平台')
  return <>
    <section className="hero section-shell">
      ...
      <div className="hero-visual">
        ...
        <FieldPreview type="stress" />
        <div className="floating-metric"><small>MAX. VON MISES</small><strong>288.0 <i>MPa</i></strong><span><i /> CONVERGED</span></div>
        <div className="visual-caption"><span>LOAD CASE · LC-01</span><span>DEFORMATION × 18.0</span><span>MESH · 5,022 DOF</span></div>
      </div>
    </section>
    ...
    {interviewer && <div className="interviewer-toast"><BriefcaseBusiness size={17} /><div><b>面试官快捷模式已开启</b><span>复杂说明已收起，建议直接体验在线仿真。</span></div><Link to="/simulate">立即体验</Link></div>}
  </>
}
...
function Simulator({ interviewer }) {
  ...
  return <section className="simulator-page">
    ...
    <div className={`sim-workspace ${interviewer ? 'simple' : ''}`}>
      <aside className="parameter-panel">
        ...
        {!interviewer && <div className="parameter-fields">{parameterSchema[model].map(([key,label,unit])=><label key={key}><span>{label}<em>{unit}</em></span><input type="number" step="any" value={params[key]} onChange={e=>setParams({...params,[key]:e.target.value})}/></label>)}</div>}
        {interviewer && <div className="quick-note"><Sparkles size={18}/><div><b>快捷预览</b><p>已使用经过验证的工程参数。点击运行即可查看完整结果。</p></div></div>}
...
function About() {
  ...
  return <><section className="about-hero section-shell"><div><Eyebrow>ABOUT / CAREER</Eyebrow><h1>CAE 工程师，<br/>也是仿真工具开发者。</h1><p>我关注的不只是求解器能否跑完，而是计算是否可信、结论能否帮助设计，以及知识能否沉淀为可复用工具。</p><div className="hero-actions"><a className="primary-button" href="mailto:phytwin@outlook.com"><Mail size={17}/>phytwin@outlook.com</a><a className="text-button" href="https://github.com/PhyTwin/PhyTwin" target="_blank" rel="noreferrer">GitHub<ExternalLink size={15}/></a></div></div><div className="profile-panel"><div className="profile-monogram">PT</div><span>CAE / MULTIPHYSICS / CODE</span><p>以第一性原理理解问题<br/>以数值方法构建模型<br/>以工程指标交付结论</p></div></section>
    <section className="section-shell value-grid">{[['01','工程建模','能把真实问题转译为正确的物理模型与边界条件。'],['02','数值计算','理解离散、收敛、误差与稳定性，而非黑盒操作。'],['03','工具开发','用 Python / React 把重复流程产品化，提升仿真效率。'],['04','技术沟通','让面试官、设计师与决策者都能读懂仿真证据。']].map(x=><div key={x[0]}><span>{x[0]}</span><h3>{x[1]}</h3><p>{x[2]}</p></div>)}</section>
