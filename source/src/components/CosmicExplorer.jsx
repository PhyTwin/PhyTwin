...
const OBJECTS = {
  solar: { index:'01', kind:'LOCAL SYSTEM', name:'太阳系', latin:'Solar System', x:1.72, y:-.76, screen:[61,57], metric:'GALACTIC RADIUS', value:'≈ 26,000 ly', description:'太阳位于猎户臂附近。放大后可观察八大行星公转与自转；轨道半径和速度经过视觉压缩，不代表真实比例。' },
...
export default function CosmicExplorer() {
  ...
  useEffect(()=>{
    ...
    const animate=()=>{frame=requestAnimationFrame(animate);const t=(performance.now()-start)/1000,active=selectedRef.current,target=active?OBJECTS[active]:null
      ...
      galaxy.scale.setScalar(THREE.MathUtils.lerp(galaxy.scale.x,active?.78:1,.04));renderer.render(scene,camera)};animate()
  ...
  return <div className={selected?'cosmic-explorer selected':'cosmic-explorer'}>
    ...
    {!selected&&<><div className="cosmic-copy"><span>PHYTWIN / GALACTIC MAP 01</span><h1>俯瞰银河，<br/>定位我们的星空。</h1><p>俯视四旋臂银河系，标记太阳系与重要星座的观测方向。点击任一标记平滑放大，探索局部三维结构。</p><div><button onClick={()=>setSelected('solar')}>放大太阳系<Maximize2 size={15}/></button><Link to="/lab">进入 CAE 实验室<ArrowRight size={15}/></Link></div></div>
      <div className="galaxy-marker-layer">{Object.entries(OBJECTS).map(([id,item])=><button key={id} className={`galaxy-marker ${id==='solar'?'solar':''}`} style={{left:`${item.screen[0]}%`,top:`${item.screen[1]}%`}} onClick={()=>setSelected(id)}><i/><span><small>{item.kind}</small><b>{item.name}</b></span><Crosshair size={13}/></button>)}</div>
      <a className="cosmic-scroll" href="#capability-map"><ArrowDown size={14}/><span>SCROLL TO ENGINEERING SCALE</span></a></>}
    {selected&&current&&<><button className="cosmic-close" onClick={()=>{setSelected(null)}}><X size={17}/>返回银河系俯视图</button><div className="cosmic-detail"><span>{current.kind} / {current.index}</span><h2>{current.name}<small>{current.latin}</small></h2><p>{current.description}</p><div className="cosmic-detail-metric"><span>{current.metric}</span><b>{current.value}</b></div><div className="cosmic-controls-hint"><Rotate3D size={15}/><span>拖动旋转局部结构</span><MousePointer2 size={14}/><span>滚轮缩放</span></div></div><div className="cosmic-model-note"><b>{selected==='solar'?'ORBITAL MOTION':'CONSTELLATION PROJECTION'}</b><span>{selected==='solar'?'行星具有独立自转与公转动画；为保证可读性，尺寸、距离与周期均采用视觉压缩。':'星座是从地球看到的方向图案，不是银河中的物理分区；总览标记为观测方向投影。'}</span></div></>}
